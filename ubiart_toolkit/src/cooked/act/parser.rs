...
use super::{
    ...
    UICarousel, UITextBox, UIWidgetElementDesc, UIWidgetGroupHUD,
...
impl<'de> BinaryDeserialize<'de> for Component<'de> {
    ...
    fn deserialize_at_with(
        ...
        let component = match component_type {
            ...
            0x8DA9_E375 => todo!("BlockFlowComponent"),
            // JD_GoldMoveComponent
            0x5632_1EA5 => todo!("GoldMoveComponent"),
            ...
            _ => {
                // 0x1528_D94A, 0x4866_6BB2, 0xA976_34C7, 0x77F7_D66C
...
impl<'de> BinaryDeserialize<'de> for UICarousel<'de> {
    ...
    fn deserialize_at_with(
        ...
        let validate_action = reader.read_at::<InternedString>(position)?;
        let carousel_data_id = reader.read_len_string_at::<u32be>(position)?;
        let (mut anim_items_desc, min_nb_items_to_loop, force_loop) = if ugi.game
            <= Game::JustDance2018
        {
            let anim_items_desc = reader.read_at_with::<CarouselAnimItemsDesc>(position, ugi)?;
            ...
            (
                Some(anim_items_desc),
                Some(min_nb_items_to_loop),
                Some(force_loop),
            )
        } else {
            (None, None, None)
        ...
        if ugi.game > Game::JustDance2018 {
            anim_items_desc = Some(reader.read_at_with::<CarouselAnimItemsDesc>(position, ugi)?);
            let unk8 = reader.read_at::<u32be>(position)?;
        ...
        if ugi.game > Game::JustDance2019
            || (ugi.game == Game::JustDance2019 && ugi.id > 0x57A7_053C)
        {
