...
//#endif
...
#if defined(ITF_WIN32) || defined(ITF_X360) || defined(ITF_WIN64) || defined(ITF_DURANGO) || defined(ITF_ORBIS)
...
#if defined(ITF_X360)    
    In.UV.x *= 0.9875f; //< depth and playerID channel contains 4 pixels of empty space on the left (for unknown reasons) 
    In.UV.x += 0.0125f; //< this is a bodge to conceal it.
#endif

    // Depth stream is DL16. Sampler is set to point. We can grab r,g,b or a.
	float depth = TEXTURE_READ_2D( TextureSampler, 0, In.UV ).r;
	float playerIDVal = ( depth % 8.0 ); // 'Mask' out to 13 bits, leaving just the bottom 3 identity bits.
	...
	float skelIdx = playerIDVal - 1;
	return ps_pipParam.vSkelColors[ skelIdx ];
#endif

#else //defined(ITF_WIN32) || defined(ITF_X360) || defined(ITF_WIN64) || defined(ITF_DURANGO) || defined(ITF_ORBIS)
	return float4( 1.f, 1.f, 1.f, 1.f );
#endif // defined(ITF_WIN32) || defined(ITF_X360) || defined(ITF_WIN64) || defined(ITF_DURANGO) || defined(ITF_ORBIS)
}
