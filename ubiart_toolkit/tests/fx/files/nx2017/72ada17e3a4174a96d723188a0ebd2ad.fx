...
struct VS_VERTSCENE_OUTPUT
{
	 ...
	 float4 oPos			: VS_OUT_POS;
};

VS_VERTSCENE_OUTPUT VertScene( VS_VERTSCENE_INPUT _in )
{
	VS_VERTSCENE_OUTPUT output;
    output.oPos = zinject( mul(_in.vPos, vs_mWorldViewProjection) );
...
float4 PixWithoutOutline( float4 color : COLOR0,
                          ...
                          float4 tex0  : TEXCOORD3 ) : PS_OUT_COLOR
{
    float4 pixel = TEXTURE_READ_2D(TextureSampler, 0, tex0.xy);
    
    // Are we rendering a colored image, or 
    // a character from only one of the channels
    if( dot(float4(1,1,1,1), chnl) )
    {
        // Get the pixel value
...
float4 PixFont( float4 color : COLOR0,
                float4 chnl  : TEXCOORD0,
                float4 tex0  : TEXCOORD3) : PS_OUT_COLOR
{
    float4 pixel = TEXTURE_READ_2D(TextureSampler, 0, tex0.xy);
...
#ifdef GRADIENT
	float colorGradient = saturate((1.0f - abs(tex0.w * 2.0f - 1.0f)) * ps_FontParam.x);
	newColor = lerp(ps_FontColorGradient, newColor, colorGradient);
#endif

#ifdef PROGRESSIVE
	newColor = ps_FontProgressiveColor;	
	float3 mask = TEXTURE_READ_2D(TextureSampler, 1, tex0.zw).rgb;
	newColor.rgb *= mask;	
	pixel.rgb -= mask - newColor.rgb;
	pixel.rgba *= color.rgba;
#else
