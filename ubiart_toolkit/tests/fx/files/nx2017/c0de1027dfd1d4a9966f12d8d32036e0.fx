...
{
	VS_PLANE_OUT output;
	output.pos = mul(input.pos, vs_mWorldViewProjection);
	output.pos.z = clamp(output.pos.z, 0.0f, output.pos.w);
	output.uv0 = input.uv0;
	output.pospostvs = output.pos;
...
VS_Particle_OUT vs_particles( VS_PC_IN input )
{
	...
	output.pos      = float4(finalPos,0,input.pos.w);
