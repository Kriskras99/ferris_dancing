...
PS_Output defaultMat_PS(VS_Output input)
{
...
#ifdef _DEBUG_
	// ps_debugParam.x = [0: mipmap, 1: mipmap (textureless)]
	
	float4 debug_mipmap_color = float4(1,1,1,1);
	float debug_mipmap_alpha = 1.0;
	
	if(ps_debugParam.x == 0.0 || ps_debugParam.x == 1.0)
	{
		if(ps_debugParam.y == 0.0)
		#ifdef _USE_LAYER1_	
			debug_mipmap_color = ComputeMipmapLevelColor(input.UV, ps_debugParam.zw);
		#else
			debug_mipmap_alpha = 0.0;
		#endif
		else if(ps_debugParam.y == 1.0)
		#ifdef _USE_LAYER2_	
			debug_mipmap_color = ComputeMipmapLevelColor(input.UV2, ps_debugParam.zw) + 0.0001 * SampleTex2D(SAMPLER_PARAM(TextureSampler, 0), input.UV2);
		#else 
			debug_mipmap_alpha = 0.0;
		#endif
		else if(ps_debugParam.y == 2.0)
		#ifdef _USE_LAYER3_	
			debug_mipmap_color = ComputeMipmapLevelColor(input.UV3, ps_debugParam.zw) + 0.0001 * SampleTex2D(SAMPLER_PARAM(TextureSampler, 0), input.UV3);
		#else 
			debug_mipmap_alpha = 0.0;
		#endif
		else if(ps_debugParam.y == 3.0)
		#ifdef _USE_LAYER4_	
			debug_mipmap_color = ComputeMipmapLevelColor(input.UV4, ps_debugParam.zw) + 0.0001 * SampleTex2D(SAMPLER_PARAM(TextureSampler, 0), input.UV4);
		#else 
			debug_mipmap_alpha = 0.0;
		#endif
	}
	...
	{
		// Show mipmap level usage
		output.Color.rgb = lerp(dot(output.Color.rgb, float3(0.11f, 0.59f, 0.3f)).xxx, debug_mipmap_color.rgb, 0.75);
		output.Color.a *= debug_mipmap_alpha;
	}
	else if(ps_debugParam.x == 1.0)
	{
		// Display the mipmap level usage as a non-transluscent color
		output.Color = debug_mipmap_color;
		output.Color.a *= debug_mipmap_alpha;
	}
	else if(ps_debugParam.x == 2.0)
	{
		output.Color = Debug_ShowLowAlpha(output.Color);
	}
#endif // _DEBUG_
