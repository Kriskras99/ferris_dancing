...
float3 getDebugColor(float4 _color, float2 uv)
{
	float3 colorOut = _color.xyz;

//Debug Color
	if(ps_debugParam.x == 1.0f)
	{    
#ifdef TEXTURE
	    float2 pixelSize = ps_debugBorder.xy * ps_debugBorder.z;
	
		//Top
		float s11 = TEXTURE_READ_2D(TextureSampler, 0, uv + float2(-1, -1)*pixelSize).a;
		float s12 = TEXTURE_READ_2D(TextureSampler, 0, uv + float2(0, -1)*pixelSize).a;
		float s13 = TEXTURE_READ_2D(TextureSampler, 0, uv + float2(1, -1)*pixelSize).a;

		//Middle
		float s21 = TEXTURE_READ_2D(TextureSampler, 0, uv + float2(-1, 0)*pixelSize).a;
		float s23 = TEXTURE_READ_2D(TextureSampler, 0, uv + float2(1, 0)*pixelSize).a;

		//bottom
		float s31 = TEXTURE_READ_2D(TextureSampler, 0, uv + float2(-1, 1)*pixelSize).a;
		float s32 = TEXTURE_READ_2D(TextureSampler, 0, uv + float2(0, 1)*pixelSize).a;
		float s33 = TEXTURE_READ_2D(TextureSampler, 0, uv + float2(1, 1)*pixelSize).a;
	    ...
	    //if(col < 0.01f)
		//	colorOut = ps_debugParam2.xyz;
		//else
			colorOut = _color.xyz;
#endif
	}
	else if(ps_debugParam.x == 2.0f)
	{
#ifdef TEXTURE

	float2 f2_TexRatio = uv * ps_debugParam.zw;
	float2 f2_dx = ddx( f2_TexRatio );
	float2 f2_dy = ddy( f2_TexRatio );
	float2 f2_UVDeriv = float2( length( f2_dx ), length( f2_dy ) );
	//f2_UVDeriv *= ps_debugParam2.xy;
	float f_mip0_1 = saturate( 2.0f - min( f2_UVDeriv.x, f2_UVDeriv.y ) );
	float f_mip1_2 = saturate( 3.0f - min( f2_UVDeriv.x, f2_UVDeriv.y ) );

	float3 colorDebug = lerp(float3(1.0f, 1.0f, 0.0f), float3(0.0f, 1.0f, 0.0f), f_mip0_1);
	colorDebug = lerp(float3(1.0f, 0.0f, 0.0f), colorDebug, f_mip1_2);
	float blackAndWhite = dot(colorOut.xyz, float3(0.11f, 0.59f, 0.3f)).xxx;
	colorOut.rgb = lerp(blackAndWhite.xxx, colorDebug, 0.75f);

#endif
	}
	else
	{
		colorOut = _color.xyz;	//lerp(_color.xyz, ps_debugParam2.xyz, ps_debugParam2.w);
	}
...
#ifdef ITF_WIN32
#ifdef _DEBUG_	
    if(ps_debugParam.x == 0.0)
    {
        float4 debug_mipmap_color = float4(1,0,0,1);
#ifdef TEXTURE        
        debug_mipmap_color = ComputeMipmapLevelColor(In.TextureUV, ps_debugParam.zw);
#endif
        result = debug_mipmap_color;
    }
	else
	{
...
{ 
	...
	result.xyz = getDebugColor(result, uv);
