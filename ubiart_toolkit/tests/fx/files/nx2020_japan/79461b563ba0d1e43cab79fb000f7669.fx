...
VS_OUT VS_Pleo( VS_IN VSIn )
{
	VS_OUT Out;
	Out.Position = mul(VSIn.Position, vs_mWorldViewProjection);
	Out.UV = VSIn.vTexture;
	Out.UV2 = VSIn.vTexture2;
...
float4 PS_Pleo(PS_IN PSIn) : PS_OUT_COLOR
{
#ifdef TEX_Y_UV
	float  Y = TEXTURE_READ_2D( TextureSampler, 0, PSIn.UV ).r;
	float cR = TEXTURE_READ_2D( TextureSampler, 1, PSIn.UV ).g;
	float cB = TEXTURE_READ_2D( TextureSampler, 1, PSIn.UV ).r;
#else
	float  Y = TEXTURE_READ_2D( TextureSampler, 0, PSIn.UV ).r;
	float cR = TEXTURE_READ_2D( TextureSampler, 1, PSIn.UV ).r;
	float cB = TEXTURE_READ_2D( TextureSampler, 2, PSIn.UV ).r;
#endif	
	...
	{
		float A = TEXTURE_READ_2D( TextureSampler, 3, PSIn.UV2 ).r;
	...
	return ARGB;
	//return float4(PSIn.UV.xy, PSIn.UV2.x, 1.0);
}
