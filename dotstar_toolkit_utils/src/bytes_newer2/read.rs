...
pub trait BinaryDeserialize<'de>: Sized {
    ...
    fn deserialize(reader: &Arc<dyn ZeroCopyReadAt<'de> + 'de>) -> Result<Self, ReadError> {
    ...
    fn read_n_at(
        ...
        let iter: LenTypeIterator<'de, Self> = LenTypeIterator {
            remaining: n,
            position,
            _type: PhantomData,
            reader: reader.clone(),
...
pub struct LenTypeIterator<'de, T>
where
...
impl<'de, T> LenTypeIterator<'de, T>
where
    T: BinaryDeserialize<'de>,
{
...
impl<'de, T> Iterator for LenTypeIterator<'de, T>
where
    T: BinaryDeserialize<'de>,
{
