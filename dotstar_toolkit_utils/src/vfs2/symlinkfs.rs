...
pub struct SymlinkFs<'fs, Fs: VirtualFileSystem> {
...
impl<Fs: VirtualFileSystem> SymlinkFs<'_, Fs> {
...
impl<Fs: VirtualFileSystem> IntoIterator for SymlinkFs<'_, Fs> {
    type Item = (PathBuf, PathBuf);

    type IntoIter = hash_map::IntoIter<PathBuf, PathBuf>;
...
impl<'fs, Fs: VirtualFileSystem> SymlinkFs<'fs, Fs> {
    /// Create a new filesystem with `backing_fs` as it's backend
    pub fn new(backing_fs: &'fs Fs) -> Self {
        Self {
            backing_fs,
            mapping: HashMap::new(),
        }
    ...
    pub fn with_capacity(backing_fs: &'fs Fs, capacity: usize) -> Self {
        Self {
            backing_fs,
...
impl<Fs: VirtualFileSystem> VirtualFileSystem for SymlinkFs<'_, Fs> {
    type VirtualFile<'fs> = Fs::VirtualFile<'fs> where Self: 'fs;
    type VirtualMetadata = Fs::VirtualMetadata;
